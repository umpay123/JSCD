...
<xsl:template match="testsuites">
    ...
    <style type="text/css">
      ...
      <![CDATA[
        function displaySystemout (name) {
          var win = window.open('','JUnitSystemProperties','scrollbars=1,resizable=1');
          var doc = win.document;
          doc.open();
          doc.write("<html><head><title>System out of " + name + "</title>");
          doc.write("<style>")
          doc.write("body {font:normal 68% verdana,arial,helvetica; color:#000000; }");
          doc.write("table tr td, table tr th { font-size: 68%; }");
          doc.write("table.properties { border-collapse:collapse; border-left:solid 1 #cccccc; border-top:solid 1 #cccccc; padding:5px; }");
          ...
          doc.write("</style>");
          doc.write("</head><body>");
          doc.write("<h3>System out of " + name + "</h3>");
          doc.write("<div align=\"right\"><a href=\"javascript:window.close();\">Close</a></div>");
          doc.write("<pre>"+sysout[name]+"</pre>");
          doc.write("</body></html>");
          doc.close();
          win.focus();
        }
    ...
    <xsl:template name="classes">
        <xsl:for-each select="testsuite">
            ...
            <div class="Properties">
                <a>
                    <xsl:attribute name="href">javascript:displaySystemout('<xsl:value-of select="@package"/>.<xsl:value-of select="@name"/>');</xsl:attribute>
                    System.out &#187;
                </a>
            </div>
            <p/>
    ...
    <xsl:template name="summary">
        ...
        <xsl:variable name="success" select="($testCount - $failureCount - $errorCount - $skippedCount)"/>
  ...
  <!-- 
    write system out into a JavaScript data structure
    This is based on idea by Tony Ding 
   -->
  <xsl:template match="system-out">
    sysout['<xsl:value-of select="../@package"/>.<xsl:value-of select="../@name"/>'] = '<xsl:call-template name="JS-escape"><xsl:with-param name="string" select="." /></xsl:call-template>';
  </xsl:template>
